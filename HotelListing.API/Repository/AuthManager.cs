...
namespace HotelListing.API.Repository
{
    public class AuthManager : IAuthManager
    {
        ...
        public async Task<IEnumerable<IdentityError>> AdminRegister(ApiUserDto userDto)
        {
            var _user = _mapper.Map<ApiUser>(userDto);
            _user.UserName = userDto.Email;
        ...
        public async Task<AuthResponseDto> VerifyRefreshToken(AuthResponseDto request)
        {
            ...
            var isValidRefreshTokenn = await _userManager.VerifyUserTokenAsync(_user, _loginProvider, _refreshToken, request.RefreshToken);

            if (isValidRefreshTokenn)
            {
                var token = await GenerateToken();
                return new AuthResponseDto
                {
